...
export default {
    ...
    methods: {
        logout() {
            Admin.logout(this.form)
                .then(() => {
                    localStorage.removeItem("token");
                    localStorage.removeItem("auth", "false");
                    localStorage.removeItem("authnticated admin")
                    this.$router.push({ name: "AdminLogin" });
                })
                .catch(err => console.error(err));
        },
        setLang() {
            localStorage.setItem("admins_lang", this.$i18n.locale);
        }
    },
    mounted() {
        this.name = JSON.parse(localStorage.getItem("authnticated admin")).name;
    }
};
</script>
<style>
